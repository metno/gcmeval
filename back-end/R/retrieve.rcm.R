retrieve.rcm <- function(ncfile,...,path=NULL,param=NULL,is=NULL,it=NULL,verbose=FALSE) {
    ...
    ncold <- ncdf4::nc_open(ncfile)

    # Extract the time information: unit and time origin
    tatt <- ncdf4::ncatt_get( ncold, varid='time' )
    ...
    vatt <- ncdf4::ncatt_get( ncold, varid=param )
    ...
    longname <- ncdf4::ncatt_get( ncold, varid=param, attname='long_name')$value
    if (is.null(longname)) {
    ...
    lat <- ncdf4::ncvar_get(ncold,varid=latid)
    lon <- ncdf4::ncvar_get(ncold,varid=lonid)
    time <- ncdf4::ncvar_get(ncold,varid='time')
    d <- c(dim(lat),length(time))
    if (verbose) print(paste('region: ',round(min(lon),digits=2),'-',round(max(lon,digits=2)),'E /',
                             round(min(lat),digits=2),'-',round(max(lat),digits=2),'N'))

    if (!is.null(is)) {
        y <- is
        if (verbose) print(paste('Use spatial coverage from an object:',floor(min(c(attr(y,"longitude")))),'-',
                                 ceiling(max(c(attr(y,"longitude")))),'E /',floor(min(c(attr(y,"latitude")))),'-',
				 ceiling(max(c(attr(y,"latitude")))),'N'))
        is <- list(lon=c(floor(min(c(attr(y,"longitude")))),ceiling(max(c(attr(y,"longitude")))),
                   lat=c(floor(min(c(attr(y,"latitude")))),ceiling(max(c(attr(y,"latitude")))))))
        rm('y')
    ...
    if (!is.null(it)) {
	    ...
	    year.time <- as.numeric(format(as.Date(time),"%Y"))
    ...
    rcm <- ncdf4::ncvar_get(ncold,varid=param,start=start, count=count)
    ncdf4::nc_close( ncold )
    if(length(dim(rcm))==3) {
      d <- dim(rcm)
    ...
    RCM <- zoo::zoo(t(rcm),order.by=time)
